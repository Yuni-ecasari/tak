...
//LIMIT 
exports.isUserLimit = (sender, isPremium, isOwner) => {
...
exports.clearAllUser = (_dir) => {
    Object.keys(_dir).forEach((i) => {
        ...
        fs.writeFileSync('./database/user.json', JSON.stringify(_dir))
    })
...
exports.cekDuluJoinAdaApaKagaMonaynyaDiJson = (sender) => {
            let status = false
            Object.keys(_db).forEach((i) => {
                if (_db[i].id === sender) {
                    status = true
  ...
  exports.addMonay = (sender, amount) => {
            let position = false
            Object.keys(_db).forEach((i) => {
                if (_db[i].id === sender) {
                    position = i
   ...
   exports.kurangMonay = (sender, amount) => {
            let position = false
            Object.keys(_db).forEach((i) => {
                if (_db[i].id === sender) {
                    position = i
...
//BALANCE/SALDO 
exports.addBalance = (sender, duit) => {
    let position = false
    Object.keys(_db).forEach((i) => {
        if (_db[i].id === sender) {
            position = i
...
exports.kurangBalance = (sender, duit) => {
    let position = false
    Object.keys(_db).forEach((i) => {
        if (_db[i].id === sender) {
            position = i
...
//GAME 
exports.isGame = (sender, isOwner, gcount) => {
    if (isOwner) {return false;}
    let found = false;
    for (let i of _db){
        if(i.id === sender){
...
exports.kurangGLimit = (sender, amount) => {
            let position = false
            Object.keys(_db).forEach((i) => {
                if (_db[i].id === sender) {
                    position = i
...
exports.tambahGLimit = (pemain, duit) => {
    let position = false
    Object.keys(_db).forEach((i) => {
        if (_db[i].id === pemain) {
            position = i
...
exports.cekGLimit = (sender, gcount) => {
    let position = false
    Object.keys(_db).forEach((i) => {
        if(_db[i].id === sender) {
            position = i
...
exports.resetLevelingXp = (sender, amount) => {
	let position = false
	Object.keys(_db).forEach((i) => {
		if (_db[i].id === sender) {
			position = i
...
//Add User ID
exports.addUserId = (gcount, tanggal, pushname, sender) => {                                                                                                                      
 obj = { name: pushname, id: sender, date: tanggal, xp: 1, level: 1, hit: 0, balance: 0, limit: limitCount, monay: monayawal, glimit: gcount }
_db.push(obj)
fs.writeFileSync('./database/user.json', JSON.stringify(_db))
}
